...
SceneManager::SceneManager(ShaderManager *pShaderManager)
{
	...
	m_basicMeshes = make_unique<ShapeMeshes>();
}
 ...
 ***********************************************************/
bool SceneManager::CreateGLTexture(const char* filename, string tag)
{
	int width = 0;
	int height = 0;
	int colorChannels = 0;
	...
	if (image)
	{
		cout << "Successfully loaded image:" << filename << ", width:" << width << ", height:" << height << ", channels:" << colorChannels << endl;
		...
		{
			cout << "Not implemented to handle image with " << colorChannels << " channels" << endl;
			return false;
		}
 ...
 ***********************************************************/
int SceneManager::FindTextureID(string tag)
{
	int textureID = -1;
	int index = 0;
	bool bFound = false;
 ...
 *  loaded texture bitmap associated with the passed in tag.
 ***********************************************************/
int SceneManager::FindTextureSlot(string tag)
{
	int textureSlot = -1;
	int index = 0;
	bool bFound = false;
 ...
 ***********************************************************/
bool SceneManager::FindMaterial(string tag, OBJECT_MATERIAL& material)
{
 ...
 ***********************************************************/
void SceneManager::SetShaderTexture(string textureTag)
{
	if (NULL != m_pShaderManager)
	{
 ...
 *  into the shader.
 ***********************************************************/
void SceneManager::SetShaderMaterial(string materialTag)
{
	if (m_objectMaterials.size() > 0)
	{
...
void SceneManager::SetupSceneLights()
{
	...
	//Directional Light
	m_pShaderManager->setVec3Value("directionalLight.position", 0.0f, 0.0f, 0.0f);
	m_pShaderManager->setVec3Value("directionalLight.ambient", 0.75f, 0.75f, 0.75f);
	...
	//Point Light 0
	m_pShaderManager->setVec3Value("pointLights[0].position", -4.0f, 0.0f, 0.0f);
	...
	//Point Light 1
	m_pShaderManager->setVec3Value("pointLights[1].position", 4.0f, 0.0f, 0.0f);
	m_pShaderManager->setVec3Value("pointLights[1].ambient", 0.9f, 0.9f, 0.9f);
	...
	//Spotlight
	m_pShaderManager->setVec3Value("spotLight.ambient", 0.0f, 2.0f, 0.0f);
	...
	m_pShaderManager->setFloatValue("spotLight.linear", 0.15f);
	m_pShaderManager->setFloatValue("spotLight.quadratic", 0.50f);
...
void SceneManager::RenderScene()
{
	...
	/****************************************************************/
	//Base of the Bowl
	scaleXYZ = glm::vec3(1.25f, 0.25f, 1.25f);
	XrotationDegrees = 0.0f;
	YrotationDegrees = 0.0f;
	ZrotationDegrees = 0.0f;
	positionXYZ = glm::vec3(0.0f, 0.5f, 2.0f);
	SetTransformations(scaleXYZ, XrotationDegrees, YrotationDegrees, ZrotationDegrees, positionXYZ);
	//SetShaderColor(1, 1, 1, 1);
	SetShaderTexture("bowlbase");
	...
	YrotationDegrees = 0.0f;
	ZrotationDegrees = 0.0f;
	positionXYZ = glm::vec3(-0.5f, 0.5f, 4.5f);
	SetTransformations(scaleXYZ, XrotationDegrees, YrotationDegrees, ZrotationDegrees, positionXYZ);
	//SetShaderColor(1, 1, 1, 1);
	SetShaderTexture("egg");
	SetTextureUVScale(1.0, 1.0);
